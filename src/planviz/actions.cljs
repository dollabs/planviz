  ...
  (let [d (st/app-get :app/loading)
    ...
    (if d
      ...
      (let [{:keys [plid update-uid state]} update
            ...
            {:keys [loaded? tpn-end]} plan-data
            {:keys [plan/corresponding] :as plan}
            (if loaded? (st/get-plan plid))]
        (if-not loaded?
          ...
          (do ;; if it has node in the uid then it's node, else edge
            (if (not (nil? (string/index-of (name update-uid) "node")))
              (let [node-id (composite-key plid update-uid)
                    ...
                    htn-net (if parent (st/get-network parent))
                    {:keys [network/parent]} htn-net
                    hem (if (= type :htn-expanded-method)
                          hpt-hent
                          (if parent (st/plans-get-node parent)))
                    hem-id (:node/id hem)]
...
(defn network-reset [plid]
  ...
  (let [plans (or (st/app-get :app/plans) {})
        plan-data (get plans plid)
        {:keys [loaded?]} plan-data
        {:keys [plan/corresponding] :as plan} (if loaded? (st/get-plan plid))]
        (if-not loaded?
...
(declare load-all-plans)

(defn login []
  (let [dlogin (tasks/connect
                 (rmethod {} :login)
                 (tasks/deferred))
        finish (-> (chain dlogin #(if (:error %) false (:login/remote %)))
      ...
      (let [[type id] c
            ...
            s (case type
                ...
                (let [edge (st/plans-get-edge id)
                      ...
                      htn-net (if parent (st/get-network parent))
                      {:keys [network/parent]} htn-net
                      ;; _ (println "htn-net parent" parent)
                      hem (if parent (st/plans-get-node parent))
                      hem-id (:node/id hem)]
                  ;; (println "HEM" hem-id)
                ...
                (let [node (st/plans-get-node id)
                      ...
                      hpt-hent (if htn-node (st/plans-get-node htn-node))
                      {:keys [node/type node/parent]} hpt-hent
                      ;; _ (println "type" type "parent" parent)
                      htn-net (if parent (st/get-network parent))
                      {:keys [network/parent]} htn-net
                      ;; _ (println "htn-net parent" parent)
                      hem (if (= type :htn-expanded-method)
...
(declare remove-subset)
(declare selection-subset)
...
(defn highlight-relevant [&[remote]]
  (let [{:keys [ui/show-plan ui/show-network ui/network-type]} (st/get-ui-opts)
        {:keys [plan/corresponding] :as plan} (st/get-plan show-plan)
...
(defn add-plan [plid plan-details]
  (let [{:keys [ui/show-plan ui/show-network ui/network-type]} (st/get-ui-opts)
    ...
    (if-not (or (= show-plan plid) (= show-plan corresponding))
      (show-plan plid))
    ))
...
(defn show-plan [plan-id &[opts]]
  ;; (println "ACTIONS/SHOW-PLAN" plan-id "OPTS" opts)
...
;; cheat! just load :sept14.tpn :sept14.htn
(defn load-all-plans [&[plan-ids]]
  ...
  (let [plan-ids (or plan-ids (vec (keys (st/app-get :app/plans))))
        ...
        (loop [prev start plan-id (first plan-ids) more (rest plan-ids)]
          (if-not plan-id
            prev
            (recur (chain prev #(load-plan plan-id))
              (first more) (rest more))))]
    (success! start true)
    finish))
...
(defn update-tpn-end [plid]
  (let [{:keys [plan/type plan/begin]} (st/get-plan plid)]
    (if (= type :tpn-network)
      (let [network (st/get-network begin)
            {:keys [network/end]} network
            plans (st/app-get :app/plans)
            plans (assoc-in plans [plid :tpn-end] end)
            ]
...
(defn load-plan [plan-id &[opts]]
  (let [d (tasks/deferred)
        dtimeout (tasks/timeout! d 10000 :timed-out)
        plans (or (st/app-get :app/plans) {})
        plan-data (get plans plan-id)
...
(defn set-selected? [new-selected? plid sel opts]
  (let [{:keys [edge node]} opts
    ...
    ;; "EDGE" (:edge/id edge) "NODE" (:node/id node))
    (case type
      ...
      :node ;; only an HTN hem node for now
  ...
  (when (and plid sel)
    (let [selection (get (st/app-get :app/selection) plid)
      ...
      (when (not= new-selection selection)
        (when (and (vector? selection) (pos? (count selection)))
          (doseq [sel selection]
            (if-not (some #(= % sel) new-selection)
              (set-selected? false plid sel opts))))
        (doseq [sel new-selection]
          (if-not (some #(= % sel) selection)
            (set-selected? true plid sel opts))))
